...
def icarl_replay(self: 'ContinualModel', dataset: 'ContinualDataset', val_set_split=0):
    ...
    if self.current_task > 0:
        # if dataset.train_loader.dataset.data[0].split('/')[1] == 'imagenet-r':
        #     print("IMAGENET-R")
        #     all_data=[]
        #     for data, label,_ in dataset.train_loader:
        #         all_data.append(data)
        # #     # print(dataset.train_loader.dataset.dataset[:][:]) 
        # #     print("first",(dataset.train_loader.dataset[0][:])) 

        #     # print("first",len(dataset.train_loader.dataset[:][:])) 

        # #     # print("second",dataset.train_loader.dataset.dataset[:][0]) 

        #     dataset.train_loader.dataset.data = deepcopy(torch.cat(all_data,dim=0))
                 #     self.val_dataset.data = self.val_dataset[:][0]
        # imgs=[]         
        # for img, label,_ in (dataset.train_loader):
        ...
        if not need_aug:
            ...
            # print(data_shape)
            # print(dataset.train_load
            #  er.dataset[0], dataset.train_loader.dataset[1])
