...
class IcarlDissectandScore:
    ...
    def dissect(self, save_dir: str):
        ...
        utils_cd.save_activations(clip_name = "ViT-B/16", protocol=self.protocol, target_name = f"{self.args.model}_{self.args.dataset}", 
                        ...
                        concept_set = f"{path}/new_exp{self.experience_number+1}_filtered_new.txt", batch_size = 200, 
                        device = "cuda", pool_mode="avg", 
                        save_dir = f"saved_activations_{self.args.model}_{self.args.dataset}")
        # total_samples = 0
        target_layer= classifier
        outputs = {"layer":[], "unit":[], "description":[], "similarity":[]}
        with open(f"{path}/new_exp{self.experience_number+1}_filtered_new.txt", 'r') as f: 
            words = (f.read()).split('\n')
            # print(len(words))
        ...
        save_names = utils_cd.get_save_names(clip_name = "ViT-B/16", target_name = f"{self.args.model}_{self.args.dataset}",
                                    ...
                                    concept_set = f"{path}/new_exp{self.experience_number+1}_filtered_new.txt", pool_mode = "avg",
                                    save_dir = f"saved_activations_{self.args.model}_{self.args.dataset}")
        ...
        if not os.path.exists(f"/home/shivank2/gokhale_user/shivanand/mammoth/activations/results_task{self.experience_number+1}_{self.args.model}_{self.args.dataset}"):
            os.mkdir(f"/home/shivank2/gokhale_user/shivanand/mammoth/activations/results_task{self.experience_number+1}_{self.args.model}_{self.args.dataset}")
        ...
        # os.mkdir(save_path)
        save_path = f"/home/shivank2/gokhale_user/shivanand/mammoth/activations/results_task{self.experience_number+1}_{self.args.model}_{self.args.dataset}"
